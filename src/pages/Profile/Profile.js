...
import {faMapMarkerAlt, faReceipt, faUser} from "@fortawesome/free-solid-svg-icons";
import {Col, Container, Row} from "react-bootstrap";
import {
    userAddressesSelector,
    userEmailSelector,
    userFullNameSelector,
    userMobileNumberSelector,
    userPastOrdersSelector
} from "../../redux/slices/userSlice";

const Profile = () => {
    const colorMode = useSelector(selectColorMode)
    const email = useSelector(userEmailSelector)
    const addresses = useSelector(userAddressesSelector)
    const fullName = useSelector(userFullNameSelector)
    const mobileNumber = useSelector(userMobileNumberSelector)
    const pastOrders = useSelector(userPastOrdersSelector)

    return (
        <div>
            <Container className=" margin_60_40">
                <Row className="justify-content-center">

                    <Col lg={4} className="">
                        <div className="box_contacts" style={{height: "25rem"}}>
                            <FontAwesomeIcon size="lg" icon={faUser} style={{fontSize: "2.5rem"}}/>
                            <h2 className={`pl-2 pb-3 d-inline-flex`}>Basic Details</h2>
                            <div className="form-group text-left">
                                <label className={`pl-1`}>Full Name</label>
                                <input className={`form-control ${colorMode}`} name="fullName"
                                       placeholder="Full Name" value={fullName}/>
                            </div>
                            <div className="form-group text-left">
                                <label className={`pl-1`}>Mobile Number</label>
                                <input className={`form-control ${colorMode}`} name="mobileNumber"
                                       placeholder="Mobile Number" value={mobileNumber}/>
                            </div>
                            <div className="form-group text-left">
                            ...
                            <div>
                                <button type="edit"
                                        className="btn_1 order_btn offers_btn order_button  ">Edit
                                </button>
                            </div>
                    ...
                    <Col lg={8} className="">
                        <div className="box_contacts " style={{overflowY: "scroll", height: '25rem'}}>
                            <FontAwesomeIcon size="lg" icon={faMapMarkerAlt} style={{fontSize: "2.5rem"}}/>
                            <h2 className={`pl-2 pb-3 d-inline-flex`}>Addresses</h2>
                            {addresses.map((item) =>
                                <div className="box_address_small"><strong>Address: </strong>

                                    {item.address} <strong>Area: </strong>{item.area.area}</div>
                            )}
                    ...
                    <Col xs={12} className="">
                        <div className="box_contacts">
                            <FontAwesomeIcon size="lg" icon={faReceipt} style={{fontSize: "2.5rem"}}/>
                            <h2 className={`pl-2 pb-3 d-inline-flex`}>Past Orders</h2>
                            {pastOrders.map((order) =>
                                <div className="box_address_small"><strong>Order ID: </strong>

                                    {order.id} <strong>Price: </strong>{order.total_price}</div>
                            )}
...
//                         <div className="progress-bar bg-primary" role="progressbar" style={{width: 80%aria-valuemax="100"}}></div>
//                       </div>
